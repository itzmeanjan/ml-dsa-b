...
static constexpr size_t BIT_SECURITY_LEVEL = ml_dsa_b_44::lambda;
static constexpr std::array<uint8_t, BIT_SECURITY_LEVEL / std::numeric_limits<uint8_t>::digits> CSPRNG_SEED{ 0 };

// Benchmark performance of ML-DSA-B-44 key generation algorithm.
void
ml_dsa_b_44_keygen(benchmark::State& state)
{
...
// Benchmark performance of ML-DSA-B-44 signing algorithm.
void
ml_dsa_b_44_sign(benchmark::State& state)
{
  ...
  for (auto _ : state) {
    ...
    has_signed &= ml_dsa_b_44::sign(rnd, seckey, msg_span, {}, sig);

    benchmark::ClobberMemory();
  }

  assert(has_signed);
  assert(ml_dsa_b_44::verify(pubkey, msg_span, {}, sig));

  state.SetItemsProcessed(state.iterations());
}

// Benchmark performance of ML-DSA-B-44 signature verification algorithm.
void
ml_dsa_b_44_verify(benchmark::State& state)
{
  const size_t mlen = state.range(0);
  ...
  ml_dsa_b_44::keygen(seed, pubkey, seckey);
  const bool has_signed = ml_dsa_b_44::sign(rnd, seckey, msg_span, {}, sig);
  assert(has_signed);
  ...
  for (auto _ : state) {
    ...
    has_verified &= ml_dsa_b_44::verify(pubkey, msg_span, {}, sig);
...
BENCHMARK(ml_dsa_b_44_keygen)->ComputeStatistics("min", compute_min)->ComputeStatistics("max", compute_max);
BENCHMARK(ml_dsa_b_44_sign)->Arg(32)->ComputeStatistics("min", compute_min)->ComputeStatistics("max", compute_max);
BENCHMARK(ml_dsa_b_44_verify)->Arg(32)->ComputeStatistics("min", compute_min)->ComputeStatistics("max", compute_max);
