...
template<size_t d>
static inline constexpr std::pair<ml_dsa_b_field::zq_t, ml_dsa_b_field::zq_t>
power2round(const ml_dsa_b_field::zq_t r)
{
  ...
  const ml_dsa_b_field::zq_t hi{ t2 };
  const ml_dsa_b_field::zq_t lo = r - ml_dsa_b_field::zq_t{ t3 };
...
static inline constexpr std::pair<ml_dsa_b_field::zq_t, ml_dsa_b_field::zq_t>
decompose(const ml_dsa_b_field::zq_t r)
{
  constexpr uint32_t t0 = alpha >> 1;
  constexpr uint32_t t1 = ml_dsa_b_field::Q - 1u;

  const uint32_t t2 = r.raw() + t0 - 1u;
  const uint32_t t3 = t2 / alpha;
  const uint32_t t4 = t3 * alpha;

  const ml_dsa_b_field::zq_t r0 = r - ml_dsa_b_field::zq_t{ t4 };
  const ml_dsa_b_field::zq_t t5 = r - r0;

  const bool flg = !static_cast<bool>(t5.raw() ^ t1);
  const ml_dsa_b_field::zq_t br[]{ ml_dsa_b_field::zq_t(t5.raw() / alpha), ml_dsa_b_field::zq_t::zero() };

  const ml_dsa_b_field::zq_t r1 = br[flg];
  const ml_dsa_b_field::zq_t r0_ = r0 - ml_dsa_b_field::zq_t{ 1u * flg };
...
highbits(const ml_dsa_b_field::zq_t r)
{
...
template<uint32_t alpha>
static inline constexpr ml_dsa_b_field::zq_t
lowbits(const ml_dsa_b_field::zq_t r)
{
  const auto s = decompose<alpha>(r);
  return s.second;
}
...
template<uint32_t alpha>
static inline constexpr ml_dsa_b_field::zq_t
make_hint(const ml_dsa_b_field::zq_t z, const ml_dsa_b_field::zq_t r)
{
  const ml_dsa_b_field::zq_t r1 = highbits<alpha>(r);
  const ml_dsa_b_field::zq_t v1 = highbits<alpha>(r + z);

  return ml_dsa_b_field::zq_t{ static_cast<uint32_t>(r1 != v1) };
}
...
template<uint32_t alpha>
static inline constexpr ml_dsa_b_field::zq_t
use_hint(const ml_dsa_b_field::zq_t h, const ml_dsa_b_field::zq_t r)
{
  constexpr uint32_t m = (ml_dsa_b_field::Q - 1) / alpha;
  constexpr ml_dsa_b_field::zq_t t0{ alpha >> 1 };
  constexpr ml_dsa_b_field::zq_t t1 = ml_dsa_b_field::zq_t{ ml_dsa_b_field::Q } - t0;
  constexpr auto one = ml_dsa_b_field::zq_t::one();

  const auto s = decompose<alpha>(r);

  if ((h == one) && ((s.second > ml_dsa_b_field::zq_t::zero()) && (s.second < t1))) {
    const bool flg = s.first == ml_dsa_b_field::zq_t{ m - 1u };
    const ml_dsa_b_field::zq_t br[]{ s.first + one, ml_dsa_b_field::zq_t{ 0u } };

    return br[flg];
  } else if ((h == one) && (s.second >= t1)) {
    const bool flg = s.first == ml_dsa_b_field::zq_t{ 0u };
    const ml_dsa_b_field::zq_t br[]{ s.first - one, ml_dsa_b_field::zq_t{ m - 1 } };
