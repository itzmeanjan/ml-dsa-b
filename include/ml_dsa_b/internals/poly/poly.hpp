...
#include "ml_dsa_b/internals/math/reduction.hpp"
...
namespace ml_dsa_b_poly {

// Given a degree-255 polynomial, this routine extracts out high and low order bits from each coefficient.
template<size_t d>
static inline constexpr void
power2round(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly,
            std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly_hi,
            std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly_lo)
  requires(ml_dsa_b_params::check_d(d))
{
  for (size_t i = 0; i < poly.size(); i++) {
    const auto ext = ml_dsa_b_reduction::power2round<d>(poly[i]);
...
static inline constexpr void
mul(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polya,
    std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyb,
    std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyc)
{
...
template<uint32_t x>
static inline constexpr void
sub_from_x(std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly)
{
  constexpr ml_dsa_b_field::zq_t x_cap(x);
...
static inline constexpr void
highbits(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> src, std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> dst)
{
  for (size_t i = 0; i < src.size(); i++) {
    dst[i] = ml_dsa_b_reduction::highbits<alpha>(src[i]);
  }
}
...
template<uint32_t alpha>
static inline constexpr void
lowbits(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> src, std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> dst)
{
  for (size_t i = 0; i < src.size(); i++) {
    dst[i] = ml_dsa_b_reduction::lowbits<alpha>(src[i]);
  }
}
...
infinity_norm(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly)
{
  constexpr ml_dsa_b_field::zq_t qby2(ml_dsa_b_field::Q / 2);
  ...
  for (size_t i = 0; i < poly.size(); i++) {
    ...
    const ml_dsa_b_field::zq_t br[]{ poly[i], -poly[i] };
...
template<uint32_t alpha>
static inline constexpr void
make_hint(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polya,
          std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyb,
          std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyc)
{
  for (size_t i = 0; i < polya.size(); i++) {
    polyc[i] = ml_dsa_b_reduction::make_hint<alpha>(polya[i], polyb[i]);
  }
}
...
template<uint32_t alpha>
static inline constexpr void
use_hint(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyh,
         std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyr,
         std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> polyrz)
{
  for (size_t i = 0; i < polyh.size(); i++) {
    polyrz[i] = ml_dsa_b_reduction::use_hint<alpha>(polyh[i], polyr[i]);
  }
}
...
count_1s(std::span<const ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly)
{
...
template<size_t d>
static inline constexpr void
shl(std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly)
{
  for (size_t i = 0; i < poly.size(); i++) {
    poly[i] = poly[i] << d;
  }
