...
static inline void
expand_a(std::span<const uint8_t, 32> rho, std::span<ml_dsa_b_field::zq_t, k * l * ml_dsa_b_ntt::N> mat)
{
  ...
  for (size_t i = 0; i < k; i++) {
    for (size_t j = 0; j < l; j++) {
      const size_t off = (i * l + j) * ml_dsa_b_ntt::N;

      msg[32] = static_cast<uint8_t>(j);
      msg[33] = static_cast<uint8_t>(i);

      auto hasher = ml_dsa_b_hashing::ml_dsa_domains::G();
      ...
      size_t n = 0;
      while (n < ml_dsa_b_ntt::N) {
        hasher.squeeze(buf_span);

        for (size_t boff = 0; (boff < buf_span.size()) && (n < ml_dsa_b_ntt::N); boff += 3) {
          ...
          if (t3 < ml_dsa_b_field::Q) {
            mat[off + n] = ml_dsa_b_field::zq_t(t3);
            n++;
          }
        }
      }
...
static inline void
expand_s(std::span<const uint8_t, 64> rho_prime, std::span<ml_dsa_b_field::zq_t, k * ml_dsa_b_ntt::N> vec)
  requires(ml_dsa_b_params::check_eta(eta) && ml_dsa_b_params::check_nonce(nonce))
{
  constexpr auto eta_value = ml_dsa_b_field::zq_t(eta);
  ...
  for (size_t i = 0; i < k; i++) {
    ...
    while (n < ml_dsa_b_ntt::N) {
      ...
      for (size_t boff = 0; (boff < buf_span.size()) && (n < ml_dsa_b_ntt::N); boff++) {
        ...
        if constexpr (eta == 2u) {
          ...
          vec[off + n] = eta_value - ml_dsa_b_field::zq_t(t2);
          n += flg0 * 1;

          const uint32_t t3 = static_cast<uint32_t>(t1 % 5);
          const bool flg1 = (t1 < 15) & (n < ml_dsa_b_ntt::N);
          const ml_dsa_b_field::zq_t br[]{ vec[off], eta_value - ml_dsa_b_field::zq_t(t3) };
          ...
          vec[off + n] = eta_value - ml_dsa_b_field::zq_t(static_cast<uint32_t>(t0));
          n += flg0 * 1;

          const bool flg1 = (t1 < 9) & (n < ml_dsa_b_ntt::N);
          const auto t2 = eta_value - ml_dsa_b_field::zq_t(static_cast<uint32_t>(t1));
          const ml_dsa_b_field::zq_t br[]{ vec[off], t2 };
...
static inline void
expand_mask(std::span<const uint8_t, 64> seed, const uint16_t nonce, std::span<ml_dsa_b_field::zq_t, l * ml_dsa_b_ntt::N> vec)
  requires(ml_dsa_b_params::check_gamma1(gamma1))
{
  constexpr size_t gamma1_bitwidth = std::bit_width(gamma1);

  std::array<uint8_t, seed.size() + 2> msg{};
  std::array<uint8_t, (ml_dsa_b_ntt::N * gamma1_bitwidth) / std::numeric_limits<uint8_t>::digits> buf{};
  ...
  for (size_t i = 0; i < l; i++) {
    ...
    ml_dsa_b_bit_packing::decode<gamma1_bitwidth>(buf_span, poly_t(vec.subspan(off, ml_dsa_b_ntt::N)));
    ml_dsa_b_poly::sub_from_x<gamma1>(poly_t(vec.subspan(off, ml_dsa_b_ntt::N)));
  }
}
...
static inline void
sample_in_ball(std::span<const uint8_t, (2 * lambda) / std::numeric_limits<uint8_t>::digits> seed, std::span<ml_dsa_b_field::zq_t, ml_dsa_b_ntt::N> poly)
  requires(ml_dsa_b_params::check_tau(tau))
{
  ...
  constexpr size_t frm = ml_dsa_b_ntt::N - tau;
  size_t i = frm;

  while (i < ml_dsa_b_ntt::N) {
    hasher.squeeze(buf_span);

    for (size_t off = 0; (off < buf_span.size()) && (i < ml_dsa_b_ntt::N); off++) {
      ...
      const ml_dsa_b_field::zq_t br0[]{ poly[i], poly[tmp] };
      const ml_dsa_b_field::zq_t br1[]{ poly[tmp], ml_dsa_b_field::zq_t::one() - ml_dsa_b_field::zq_t(2u * s_) };
