#pragma once
#include "ml_dsa_b/internals/ml_dsa_b.hpp"

namespace ml_dsa_b_65 {
...
static constexpr uint32_t gamma2 = (ml_dsa_b_field::Q - 1) / 32;
...
static constexpr size_t KeygenSeedByteLen = ml_dsa_b::KEYGEN_SEED_BYTE_LEN;

// Byte length ( = 1952 ) of ML-DSA-65 public key.
static constexpr size_t PubKeyByteLen = ml_dsa_b_utils::pub_key_len(k, d);

// Byte length ( = 4032 ) of ML-DSA-65 secret key.
static constexpr size_t SecKeyByteLen = ml_dsa_b_utils::sec_key_len(k, l, eta, d);

// Byte length ( = 32 ) of ML-DSA-65 signing seed.
static constexpr size_t SigningSeedByteLen = ml_dsa_b::RND_BYTE_LEN;

// Byte length ( = 3309 ) of ML-DSA-65 signature.
static constexpr size_t SigByteLen = ml_dsa_b_utils::sig_len(k, l, gamma1, omega, lambda);
...
{
  ml_dsa_b::keygen<k, l, d, eta>(ξ, pubkey, seckey);
}
...
{
  return ml_dsa_b::sign<k, l, d, eta, gamma1, gamma2, tau, beta, omega, lambda>(rnd, seckey, msg, ctx, sig);
}
...
sign_internal(std::span<const uint8_t, SigningSeedByteLen> rnd,
              ...
              std::span<uint8_t, SigByteLen> sig)
{
  return ml_dsa_b::sign_internal<k, l, d, eta, gamma1, gamma2, tau, beta, omega, lambda>(rnd, seckey, mu, sig);
}
...
{
  return ml_dsa_b::verify<k, l, d, gamma1, gamma2, tau, beta, omega, lambda>(pubkey, msg, ctx, sig);
}
...
{
  return ml_dsa_b::verify_internal<k, l, d, gamma1, gamma2, tau, beta, omega, lambda>(pubkey, mu, sig);
}
