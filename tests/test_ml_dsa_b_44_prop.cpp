...
static void
test_ml_dsa_b_44_signing(const size_t mlen, const size_t ctx_len)
{
  assert(ctx_len < 256);

  std::array<uint8_t, ml_dsa_b_44::KeygenSeedByteLen> seed{};
  std::array<uint8_t, ml_dsa_b_44::PubKeyByteLen> pkey{};
  std::array<uint8_t, ml_dsa_b_44::PubKeyByteLen> pkey_copy{};
  ...
  std::array<uint8_t, ml_dsa_b_44::SigByteLen> sig_copy{};
  ...
  ml_dsa_b_44::keygen(seed, pkey, skey);                              // Generate a valid ML-DSA-44 keypair
  EXPECT_TRUE(ml_dsa_b_44::sign(rnd, skey, msg_span, ctx_span, sig)); // Sign a random message with ML-DSA-44 secret ket
  ...
  EXPECT_TRUE(ml_dsa_b_44::verify(pkey, msg_span, ctx_span, sig));       // pkey is good, msg is good, ctx is good, sig is good
  EXPECT_FALSE(ml_dsa_b_44::verify(pkey, msg_span, ctx_span, sig_copy)); // pkey is good, msg is good, ctx is good, sig is bad
  EXPECT_FALSE(ml_dsa_b_44::verify(pkey_copy, msg_span, ctx_span, sig)); // pkey is bad, msg is good, ctx is good, sig is good

  if (mlen > 0) {
    std::copy(msg_span.begin(), msg_span.end(), msg_copy_span.begin());
    ml_dsa_test_helper::random_bit_flip(msg_copy_span);

    EXPECT_FALSE(ml_dsa_b_44::verify(pkey, msg_copy_span, ctx_span, sig)); // pkey is good, msg is bad, ctx is good, sig is good
  }
  if (ctx_len > 0) {
    std::copy(ctx_span.begin(), ctx_span.end(), ctx_copy_span.begin());
    ml_dsa_test_helper::random_bit_flip(ctx_copy_span);

    EXPECT_FALSE(ml_dsa_b_44::verify(pkey, msg_span, ctx_copy_span, sig)); // pkey is good, msg is good, ctx is bad, sig is good
  }
}

TEST(ML_DSA_B, ML_DSA_B_44_KeygenSignVerifyFlow)
{
  for (size_t mlen = 0; mlen < 33; mlen++) {
    for (size_t ctx_len = 0; ctx_len < 256; ctx_len++) {
      test_ml_dsa_b_44_signing(mlen, ctx_len);
    }
  }
}
